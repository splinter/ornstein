...
var buildRequestOptions = function(definition) {
    ...
    var queryString = query.reduce(function(previous, current) {
...
var printRequestDetails = function(options, definition, response) {
    ...
    msgs.push({
        msg: definition.scheme.name + '://' + options.host + ':' + options.port + '/' + options.path,
...
var invokeDHCRequest = function(definition, projectName) {
    ...
    var req = http.request(opts, function(response) {
        ...
        response.on('end', function() {
            ...
            console.log(chalk.grey(JSON.stringify(jsonResp, null, 4)));
            console.log(chalk.grey('Headers:'));
            console.log(chalk.grey(JSON.stringify(response.headers, null, 4)));
            recordRequestResponse(definition.name,jsonResp);
            recordStatus(definition.name,response.statusCode);
            if (response.statusCode < 400) {
                notify.info('Starting testing');
                recordRequestSuccess(definition.name);
    ...
    req.on('error', function() {
        recordStatus(definition.name,"Unable to contact endpoint");
        recordRequestFailure(definition.name);
...
var invokeDHCTests = function(requestName, projectName, definition, response) {
    ...
    tests.forEach(function(testName) {
        ...
        var result = executeTestScript(script, definition, response);
        recordRequestTests(requestName,result);
    });
};
...
var recordRequestFailure = function(requestName){
    var ptr = resultSet[requestName];
    if(!resultSet.hasOwnProperty(requestName)){
        ptr = resultSet[requestName] = {};
    }
    ptr.passed = false;
};
var recordRequestSuccess = function(requestName){
    var ptr = resultSet[requestName];
    if(!resultSet.hasOwnProperty(requestName)){
        ptr = resultSet[requestName] = {};
    }
...
var recordRequest = function(requestName, details) {
    var ptr = resultSet[requestName];
    if (!resultSet.hasOwnProperty(requestName)) {
        ptr = resultSet[requestName] = {};
    }
...
var recordRequestResponse = function(requestName,response){
    resultSet[requestName].response = JSON.stringify(response,null,4);
};
var recordRequestTests = function(requestName, tests) {
    var ptr = resultSet[requestName];
    ptr.tests = tests;
};
var recordStatus = function(requestName,status){
    resultSet[requestName].status = status;
};
